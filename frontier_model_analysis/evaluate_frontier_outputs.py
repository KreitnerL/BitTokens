...
PROJECT_PATH = os.path.join(".", "frontier_model_analysis", "results")
...
FILENAME_RE = re.compile(
    r"^(?P<task>[A-Za-z]+)_decimal_uniform_test_500_shuffle_(?P<model>.+?)_re_(?P<reasoning>[A-Za-z]+)\.csv$"
)
...
def make_radar_charts(df_all: pd.DataFrame, out_dir: Path) -> None:
    ...
    """
    # Aggregate performance; merge Qwen variants by canonical key
...
def make_tokens_matrix(df_all: pd.DataFrame, out_dir: Path) -> None:
    ...
    df_filt = df_all[df_all["tokens"] != 65535]
...
def main() -> None:
    ...
    for task in task_set:
        canon_path = data_dir / f"{task}_decimal_uniform_test_500_shuffle.csv"
    ...
    perf_by_mrt = df_all.groupby(["model", "reasoning", "task"], as_index=False).agg(
        mean_performance=("performance", "mean")
    )
    ...
    coverage = (
        df_all.groupby(["model", "reasoning"])
        .agg(
