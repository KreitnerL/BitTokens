...
for implementation in "${!implementations[@]}"; do
    ...
    sbatch --job-name="MultiTask_hard_${IMPLEMENTATION}_${suffix}" --output=$PROJECT_PATH/slurm/wandb/multitask_hard-$IMPLEMENTATION-$suffix-%A.out --error=$PROJECT_PATH/slurm/wandb/multitask_hard-$IMPLEMENTATION-$suffix-%A.err $PROJECT_PATH/slurm_scripts/multi_task_hard.sbatch "$IMPLEMENTATION" "$PARAMS"
    sleep 1
done
